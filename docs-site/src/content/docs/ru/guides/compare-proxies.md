...
**Research pin (v1):** для небольшого флота (2–5 своих логинов ChatGPT) с закреплением воркера
отправьте `x-ocx-codex-account: <id|main>` или запустите `ocx codex --account …`. Пин отключает
автопереключение и fail-closed при 429/reauth; без пина остаётся поведение пула квот.

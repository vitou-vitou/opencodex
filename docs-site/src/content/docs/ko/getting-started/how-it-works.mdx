...
:::note[병렬 플릿이 아님]
이 풀은 **하나의 요청 경로 위 할당량 풀**입니다. 기존 thread는 affinity를 유지하고, 새 세션은 사용량에
따라 failover·자동 전환할 수 있습니다. 계정마다 동시 Codex 세션을 N개 돌리는 **병렬 플릿이 아닙니다**.
워커마다 고정하려면 헤더 `x-ocx-codex-account` 또는 `ocx codex --account <id|email|main>`으로 핀하세요
(429/재인증 시 페일 클로즈, 자동 전환 없음). 같은 구분은
[프록시 비교](/ko/guides/compare-proxies/)를 보세요.
:::
